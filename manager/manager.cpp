...
manager::manager(char *config, bool print) {
  ...
  verbosity = print;
  if (verbosity) std::cout << std::endl << "Reading configuration from " << config << std::endl;
  ...
  if (success == EXIT_FAILURE) {
    std::cerr << "Something went wrong in the manager, check above error message and your config file" << std::endl;
    is_good_config = false;
    exit(EXIT_FAILURE);
  } else {
    is_good_config = true;
    if (verbosity) std::cout << "Succesfully read config " << config << ", now proceeding" << std::endl;
  }
...
int manager::readConfig(char *config) {
  ...
  // Any crucial settings should hence not be wrapped in the if(cfg.exists("PARNAME"))
  try {
    ...
    if (cfg.exists("NBURNINSTEPS")){
      ...
      n_burnin_steps = 1000;
      if (verbosity) std::cout << "Number of burn in steps not specified, using " << n_burnin_steps << std::endl;
    }
    ...
    if (cfg.exists("POT")) {
      ...
      if (verbosity) std::cout << "- no POT specified, setting to 3.01E20" << std::endl;
    }
    ...
    if (cfg.exists("NUBARPOT")) {
      ...
      if (verbosity) std::cout << "- no nubar POT specified, setting to 3.01E20" << std::endl;
    }
    ...
    if (cfg.exists("XAXISBINS")) {
	  ...
	  std::cout << "[WARNING] - now x-axis bins specified. Using 0 and 1 as default" << std::endl;
      xaxisbins.push_back(0);
      xaxisbins.push_back(1);
    }
    ...
    if (cfg.exists("YAXISBINS")) {
	  ...
	  std::cout << "[WARNING] - now y-axis bins specified. Using 0 and 1 as default" << std::endl;
      yaxisbins.push_back(0);
      yaxisbins.push_back(1);
    }
    ...
    if(cfg.exists("XSECCOVFILE")) {
      ...
      if (verbosity) std::cout << "- found xsecCovMatrix = " << xsec_cov_file << std::endl;
    } else {
	  //TODO (ETA) - support not specifiying xsec cov if xsec systs are turned off in xsec_syst_opt
	  std::cerr << "[ERROR] - no xsec covariance matrix specified which is required!" << std::endl;
	  throw;
    }
    ...
    if(cfg.exists("XSECCOVNAME")) {
      ...
      if (verbosity) std::cout << "- found xsec covariance name = " << xsec_cov_name << std::endl;
    } else {
      xsec_cov_name = "xsec_cov";
      if (verbosity) std::cout << "- didn't find xsec covariance matrix name! Setting default = " << xsec_cov_name << std::endl;
    }

    // Look for specified ND280 covariance
    if (cfg.exists("NEARDETCOVFILE")) {
      near_det_cov_file = (const char*)cfg.lookup("NEARDETCOVFILE");
      if (verbosity) std::cout << "- found Near detector covariance matrix file = " << near_det_cov_file << std::endl;
    } else {
	  //TODO (ETA) - support not specifiying near detector covariance matrix if near det systs are turned off in near_det_syst_opt
	  std::cerr << "[ERROR] - no near detector covariance matrix specified which is required!" << std::endl;
	  throw;
    }
    ...
    if(cfg.exists("NEARDETCOVNAME")) {
      ...
      if (verbosity) std::cout << "- found Near detector covariance matrix name = " << near_det_cov_name << std::endl;
    } else {
      near_det_cov_name = "near_det_cov";
      if (verbosity) std::cout << "- didn't find Near detector covariance matrix name! Setting default = " << near_det_cov_name << std::endl;
    }
    ...
    if(cfg.exists("FARDETCOVFILE")) {
	  ...
	  //TODO (ETA) - support not specifiying far detector covariance matrix if far det systs are turned off in far_det_syst_opt
	  std::cerr << "[ERROR] - no far detector covariance matrix specified which is required!" << std::endl;
	  throw;
    ...
    if(cfg.exists("FARDETCOVNAME")) {
      ...
      if (verbosity) std::cout << "- found Far detector covariance matrix name = " << far_det_cov_name << std::endl;
    } else {
      far_det_cov_name = "far_det_cov";
      if (verbosity) std::cout << "- didn't find Far detector covariance matrix name! Setting default = " << far_det_cov_name << std::endl;
    }

    // Where is osc covariance
    if(cfg.exists("OSCCOVFILE")) {
      osc_cov_file = (const char*)cfg.lookup("OSCCOVFILE") ;
      if (verbosity) std::cout << "- found oscCovMatrix = " << osc_cov_file << std::endl;
    } else {
      osc_cov_file = "inputs/oscillation_covariance_6par_nondouble.root";
      if (verbosity) std::cout << "- didn't find oscCovMatrix! Setting default = " << osc_cov_file << std::endl;
    }

    // The name of the matrix to use
    if(cfg.exists("OSCCOVNAME")) {
      osc_cov_name = (const char*)cfg.lookup("OSCCOVNAME") ;
      if (verbosity) std::cout << "- found oscCovMatrix name = " << osc_cov_name << std::endl;
    } else {
      osc_cov_name = "osc_cov";
      if (verbosity) std::cout << "- didn't find oscCovMatrix! Setting default = " << osc_cov_name << std::endl;
    }
    ...
    if(cfg.exists("OSCCOVFILE_ANTINU")) {
      ...
      if (verbosity) std::cout << "- found oscCovMatrix_antinu = " << osc_cov_antinu_file << std::endl;
    } else {
      osc_cov_antinu_file = "osc_cov";
    }

    // The name of the matrix to use
    if(cfg.exists("OSCCOVNAME_ANTINU")) {
      osc_cov_antinu_name = (const char*)cfg.lookup("OSCCOVNAME_ANTINU") ;
      if (verbosity) std::cout << "- found oscCovMatrix_antinu name = " << osc_cov_antinu_name << std::endl;
    } else {
      osc_cov_antinu_name = "";
    }
    ...
    if (cfg.exists("STATONLY")) {
      ...
      if (verbosity) std::cout << "- found STAT only, setting " << is_stat_only << std::endl;
    } else {
      is_stat_only = 0;
      if (verbosity) std::cout << "- didn't find STATONLY, setting default " << is_stat_only << std::endl;
    }

    // Do oscillation?
    if (cfg.exists("DOOSC")) {
      do_osc = cfg.lookup("DOOSC");
      if (verbosity) std::cout << "- found DOOSC, setting " << do_osc << std::endl;
    } else {
      do_osc = 0;
      if (verbosity) std::cout << "- didn't find DOOSC, setting default " << do_osc << std::endl;
    }

    // Is this statistics only fit?
    if (cfg.exists("USEFARSYST")) {
      use_far_syst = cfg.lookup("USEFARSYST");
      if (verbosity) std::cout << "- found USEFARSYST, setting " << use_far_syst << std::endl;
    } else {
      use_far_syst = true;
      if (verbosity) std::cout << "- didn't find USEFARSYST, setting default " << use_far_syst << std::endl;
    }

    // What far detector params have flat priors ("array" in libconfig, std::vector<int> in class)
    if (cfg.exists("FARDETPARAMFLAT")) {
      const libconfig::Setting &root = cfg.getRoot();
      const libconfig::Setting &setting = root["FARDETPARAMFLAT"];
      for (int i = 0; i < int(setting.getLength()); i++)
    ...
    // Decide whether beta should be applied to nue (true) appearance probability instead of nuebar
    ...
    // Decide wheter you should apply (1/beta) to nue appearance probability and beta to nuebar appearance probability (true)
  ...
  // Catch if the config file doesn't contain some crucial parameter which we haven't wrapped in exist
  catch (const libconfig::SettingNotFoundException &nfex) {
    std::cerr << "Setting " << nfex.getPath() << " not found in file " << config << std::endl;
    return(EXIT_FAILURE);
  }
  ...
  // If invalid type conversion (e.g. char* to double) - Catches if we give FLUXCOVFILE=-1 or suttin
